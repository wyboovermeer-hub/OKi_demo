...
# Changelog v21.29
# -----------------
# • Vessel Logbook — /log page and /api/log endpoint live
#   SQLite-backed persistent event log with filters, stats bar,
#   deep cycle count, level badges, auto-refresh every 30s
# • /api/log/clear POST endpoint (dev only)
#
# Changelog v21.28
# -----------------
...
try:
    ...
    _LOGBOOK_AVAILABLE = True
except Exception as _lb_err:
    print(f"[OKi] Warning — logbook import failed: {_lb_err}")
    _LOGBOOK_AVAILABLE = False
import sys
import os
...
def render_footer():
    ...
    return (
        ...
        # Progress ring SVG
        '<svg id="logo-ring" width="132" height="132" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:none;opacity:0;transition:opacity 0.3s;">'
        '<circle cx="66" cy="66" r="62" fill="none" stroke-width="3" stroke="url(#ringGrad)" stroke-linecap="round"'
        ' stroke-dasharray="389.6" stroke-dashoffset="389.6" id="ring-arc"'
        ' style="transform:rotate(-90deg);transform-origin:66px 66px;transition:stroke-dashoffset 0.05s linear"/>'
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  body::before {
    ...
    background: linear-gradient(90deg, var(--plasma-1) 0%, var(--plasma-2) 25%,
  ...
  @keyframes plasma-shift { 0% { filter: hue-rotate(0deg); } 100% { filter: hue-rotate(360deg); } }

  .page { position: relative; z-index: 2; max-width: 1100px; margin: 0 auto; padding: 24px 20px 60px; }

  .header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 28px; padding-bottom: 16px; border-bottom: 1px solid rgba(84,139,172,0.25); }
  .header-left { display: flex; align-items: center; gap: 14px; }
  .header-left a { color: var(--wmc-mid); text-decoration: none; font-family: var(--mono); font-size: 11px; letter-spacing: 0.1em; opacity: 0.7; transition: opacity 0.2s; }
  .header-left a:hover { opacity: 1; color: var(--wmc-light); }
  .title { font-family: var(--mono); font-size: 22px; letter-spacing: 0.15em; color: var(--wmc-light); text-transform: uppercase; }
  .title span { background: linear-gradient(90deg, var(--plasma-4), var(--plasma-5)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
  .subtitle { font-size: 11px; font-family: var(--mono); color: var(--wmc-mid); opacity: 0.6; margin-top: 3px; letter-spacing: 0.1em; }

  .stats-bar { display: flex; gap: 16px; margin-bottom: 20px; flex-wrap: wrap; }
  .stat-pill { background: rgba(84,139,172,0.08); border: 1px solid rgba(84,139,172,0.2); border-radius: 20px; padding: 6px 14px; font-family: var(--mono); font-size: 11px; color: var(--wmc-mid); display: flex; align-items: center; gap: 6px; }
  .stat-pill .num { font-size: 14px; font-weight: 600; color: var(--wmc-light); }
  ...
  .filter-label { font-family: var(--mono); font-size: 10px; color: var(--wmc-mid); opacity: 0.6; letter-spacing: 0.1em; text-transform: uppercase; margin-right: 4px; }
  .filter-btn { background: rgba(255,255,255,0.04); border: 1px solid rgba(84,139,172,0.2); border-radius: 4px; padding: 5px 12px; font-family: var(--mono); font-size: 10px; letter-spacing: 0.08em; color: var(--wmc-mid); cursor: pointer; transition: all 0.15s; text-transform: uppercase; }
  .filter-btn:hover { border-color: var(--wmc-blue); color: var(--wmc-light); }
  .filter-btn.active { border-color: var(--wmc-blue); background: rgba(84,139,172,0.15); color: var(--wmc-light); }

  .log-table { width: 100%; border-collapse: collapse; font-family: var(--mono); font-size: 12px; }
  .log-table thead th { text-align: left; padding: 8px 12px; font-size: 10px; letter-spacing: 0.12em; text-transform: uppercase; color: var(--wmc-mid); opacity: 0.6; border-bottom: 1px solid rgba(84,139,172,0.2); }
  .log-row { border-bottom: 1px solid rgba(84,139,172,0.08); transition: background 0.15s; animation: row-in 0.3s ease forwards; opacity: 0; }
  ...
  .log-row td { padding: 10px 12px; vertical-align: top; }

  .col-time { color: var(--wmc-mid); opacity: 0.55; white-space: nowrap; font-size: 11px; width: 155px; }

  .level-badge { display: inline-block; padding: 2px 7px; border-radius: 3px; font-size: 9px; letter-spacing: 0.1em; font-weight: 700; text-transform: uppercase; white-space: nowrap; }
  ...
  .level-MAYDAY   { background: rgba(255,45,120,0.25); color: #ff2d78; border: 1px solid rgba(255,45,120,0.6); animation: mayday-pulse 1s ease-in-out infinite; }
  @keyframes mayday-pulse { 0%,100% { box-shadow: 0 0 4px rgba(255,45,120,0.4); } 50% { box-shadow: 0 0 12px rgba(255,45,120,0.8); } }

  .cat-badge { display: inline-block; padding: 2px 6px; border-radius: 3px; font-size: 9px; letter-spacing: 0.08em; text-transform: uppercase; background: rgba(84,139,172,0.1); color: var(--wmc-mid); border: 1px solid rgba(84,139,172,0.2); }
  .cat-BATTERY  { background: rgba(48,209,88,0.08);  color: #30d158; border-color: rgba(48,209,88,0.3); }
  .cat-SEVERITY { background: rgba(255,45,120,0.08); color: #ff2d78; border-color: rgba(255,45,120,0.3); }
  .cat-CARE     { background: rgba(10,132,255,0.08); color: #0a84ff; border-color: rgba(10,132,255,0.3); }
  .cat-SCENARIO { background: rgba(191,90,242,0.08); color: #bf5af2; border-color: rgba(191,90,242,0.3); }
  .cat-SYSTEM   { background: rgba(84,139,172,0.10); color: #81a4c4; border-color: rgba(84,139,172,0.3); }

  .col-title { color: var(--wmc-light); max-width: 500px; }
  .col-detail { font-size: 10px; color: var(--wmc-mid); opacity: 0.7; margin-top: 3px; }
  .batt-tag { display: inline-block; margin-top: 4px; padding: 1px 5px; border-radius: 2px; font-size: 9px; background: rgba(48,209,88,0.1); color: #30d158; border: 1px solid rgba(48,209,88,0.25); }
  .col-value { text-align: right; white-space: nowrap; font-size: 13px; font-weight: 600; }
  .val-soc  { color: var(--wmc-mid); }
  .val-soh  { color: var(--plasma-3); }
  .val-low  { color: var(--plasma-1); }

  .empty { text-align: center; padding: 60px 20px; color: var(--wmc-mid); opacity: 0.4; font-family: var(--mono); font-size: 13px; letter-spacing: 0.1em; }
  .loading { text-align: center; padding: 40px; font-family: var(--mono); font-size: 12px; color: var(--wmc-mid); opacity: 0.5; letter-spacing: 0.1em; }

  .refresh-btn { background: rgba(84,139,172,0.1); border: 1px solid rgba(84,139,172,0.3); border-radius: 4px; padding: 7px 16px; font-family: var(--mono); font-size: 10px; letter-spacing: 0.1em; color: var(--wmc-mid); cursor: pointer; text-transform: uppercase; transition: all 0.15s; }
  .refresh-btn:hover { border-color: var(--wmc-blue); color: var(--wmc-light); background: rgba(84,139,172,0.18); }
...
<div class="page">
  ...
  <div class="stats-bar" id="stats-bar">
    <div class="stat-pill"><span>Total events</span><span class="num" id="stat-total">—</span></div>
    <div class="stat-pill deep"><span>Deep cycles</span><span class="num" id="stat-deep">—</span></div>
    <div class="stat-pill soh"><span>SoH readings</span><span class="num" id="stat-soh">—</span></div>
    <div class="stat-pill"><span>Last event</span><span class="num" id="stat-last">—</span></div>
  </div>
  <div class="filters">
...
<script>
  (function() {
    ...
    function resize() {
      ...
      stars = Array.from({length: 180}, () => ({x: Math.random()*c.width, y: Math.random()*c.height, r: Math.random()*1.2+0.2, o: Math.random()*0.5+0.1}));
    }
    function draw() {
      ctx.clearRect(0,0,c.width,c.height);
      stars.forEach(s => { ctx.beginPath(); ctx.arc(s.x,s.y,s.r,0,Math.PI*2); ctx.fillStyle=`rgba(202,216,227,${s.o})`; ctx.fill(); });
    }
    resize(); draw();
    window.addEventListener('resize', () => { resize(); draw(); });
  })();

  let allEntries = [], currentCat = '', currentLevel = '';

  async function loadLog() {
    document.getElementById('log-container').innerHTML = '<div class="loading">Retrieving vessel memory...</div>';
    try {
      ...
      if (allEntries.length > 0) document.getElementById('stat-last').textContent = formatRelative(allEntries[0].timestamp);
      render();
    } catch(e) {
      document.getElementById('log-container').innerHTML = '<div class="empty">No logbook data available.<br>The vessel memory begins when OKi starts recording.</div>';
    }
  }
  ...
  function render() {
    const filtered = allEntries.filter(e => {
      ...
      if (currentLevel) { const order = {INFO:0,WARNING:1,ALERT:2,CRITICAL:3,MAYDAY:4}; if ((order[e.level]||0) < (order[currentLevel]||0)) return false; }
      return true;
    });
    if (filtered.length === 0) { document.getElementById('log-container').innerHTML = '<div class="empty">No events match the current filter.</div>'; return; }
    let rows = filtered.map(e => {
      const ts  = formatTimestamp(e.timestamp);
      const val = e.value_num !== null && e.value_num !== undefined ? `<span class="${valueClass(e)}">${e.value_num.toFixed(1)}${e.value_unit ? ' '+e.value_unit : ''}</span>` : '';
      const battTag = e.battery_id ? `<div class="batt-tag">${e.battery_id}</div>` : '';
      const detail  = e.detail ? `<div class="col-detail">${e.detail}</div>` : '';
      return `<tr class="log-row"><td class="col-time">${ts}</td><td><span class="level-badge level-${e.level}">${e.level}</span></td><td><span class="cat-badge cat-${e.category}">${e.category}</span></td><td class="col-title">${e.title}${detail}${battTag}</td><td class="col-value">${val}</td></tr>`;
    }).join('');
    document.getElementById('log-container').innerHTML = `<table class="log-table"><thead><tr><th>Timestamp</th><th>Level</th><th>Category</th><th>Event</th><th style="text-align:right">Value</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  ...
  function formatTimestamp(iso) {
    try { const d = new Date(iso); return d.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})+' '+d.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',second:'2-digit'}); }
    catch { return iso; }
  }

  function formatRelative(iso) {
    try { const diff = Date.now()-new Date(iso).getTime(); const m=Math.floor(diff/60000); if(m<1) return 'just now'; if(m<60) return m+'m ago'; const h=Math.floor(m/60); if(h<24) return h+'h ago'; return Math.floor(h/24)+'d ago'; }
    catch { return '—'; }
  }
...
async def api_log(
    ...
    if not _LOGBOOK_AVAILABLE:
        return JSONResponse({"entries": [], "total": 0, "deep_cycle_count": 0, "error": "logbook module not available"})
    categories = [category] if category else None
    entries = lb.get_all_entries(limit=limit, categories=categories, min_level=level)
    deep_cycles = lb.get_deep_cycle_count()
    return JSONResponse({"entries": entries, "total": len(entries), "deep_cycle_count": deep_cycles})

@app.post("/api/log/clear")
async def api_log_clear():
    if not _LOGBOOK_AVAILABLE:
        return {"status": "logbook not available"}
    lb.clear_all()
